...
filenames =[]
with open('TransferLearningModelResult.csv', 'w', newline='') as writeFile:
    ...
    for i in range(6):
        ...
        for filename in os.listdir(test_dir + x + '_def'):
            ...
            writer.writerow(filenames)
writeFile.close()
submit = pd.read_csv('TransferLearningModelResult.csv')
submit['defect'] = predict
submit.to_csv('TransferLearningModelResult.csv', index=False)
print("Results are saved in TransferLearningModelResult csv file")
print("Evaluating Transfer learning model execution is completed")
